...
describe("PenHub observation and parsers", () => {
  ...
  test("parses structured tool outputs", () => {
    ...
    expect(parseFfufJson(JSON.stringify({ results: [{ url: "http://local/admin", status: 200, length: 12 }] }))).toEqual([
      { url: "http://local/admin", status: 200, length: 12 },
    ])
    expect(parseNucleiJsonl('{"template-id":"sample","info":{"severity":"info","name":"Sample"},"matched-at":"http://local"}\n')).toEqual([
      { templateId: "sample", severity: "info", matchedAt: "http://local", name: "Sample" },
    ])
  })
})
